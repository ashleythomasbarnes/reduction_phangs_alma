...
# $Id: almaqa2isg.py,v 2.6 2023/05/16 10:30:05 dpetry Exp $
#
"""
...
def version(short=False):
    ...
    """
    myversion = "$Id: almaqa2isg.py,v 2.6 2023/05/16 10:30:05 dpetry Exp $"
    if (short):
...
def generateImagingScript(vis="", draft_threshold="", reqchanwidth=1, makecubes=True, docontsub=None, chanwidthtol=0.05,
    ...
    if docontsub == None:
        if addfullpolanalysis:
            ...
            docontsub = True
    elif docontsub == True and addfullpolanalysis and aU.getCasaVersion() < '6.5.2':
        print("ERROR: docontsub=True on full-pol data will only work with CASA version 6.5.2 or later (CAS-12956).")
        return False
    ...
    for myintent in additionalimg:
        ...
        for myfield in additionalimg_fields[myintent]:
            ...
            for adtldict in adtlfielddicts:
                ...
                #bring the set vissforfields[myfield] into chronological order
                myvissforfield = []
                for mvis in myviss:
    ...
    mypardict['imagename'] = "VARmyimagebase+'.mask'"
    mypardict['fitsimage'] = "VARmyimagebase+'.mask.fits'"

    stext = stext +  mystepindent + "if os.path.exists(myimagebase+'.mask'):\n"
...
def getimgpars(vis='', fieldname='', intent='', mode='', isMosaic=False, isEphem=False, isFullPol=False, field=[], spw=[], 
    ...
    try:
        thecellsize, theimsize, thephasecenter = aU.pickCellSize(vis=thevis, 
                                                                 ...
                                                                 verbose=False)
        if isMosaic:
            tmpdict = aU.plotmosaic(vis=thevis, figfile=thevis+'_'+thefieldname+'_mosaic-layout.png', sourceid=thefieldname,
                                    field=thefieldids)
            thephasecenter = tmpdict[0]
...
def printfullpolanalysis():
    ...
    rval += "  results[inimageIQUV]['fluxPI'] = math.sqrt(fluxQ**2+fluxU**2)\n"

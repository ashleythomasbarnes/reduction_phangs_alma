...
PLOTBANDPASS_REVISION_STRING = "$Id: plotbandpass3.py,v 2.6 2023/04/15 21:59:45 thunter Exp $" 
import pylab as pb
...
def version(showfile=True):
    ...
    """
    myversion = "$Id: plotbandpass3.py,v 2.6 2023/04/15 21:59:45 thunter Exp $" 
    if (showfile):
...
#      print("Building %s with dpi=%d" % (plotfilename, density))
      print("Building %s" % (plotfilename))
  figdesc = pb.gcf()
  pb.savefig(plotfilename, format='png') # , dpi=density) # deprecated in matplotlib of casa 6.5.3
  return(plotfilename)
...
#        print("start=%d, stop=%d, len(solution)=%d" % (start,stop,len(solution)))
    ...
    def func_old_style(*args, **kwargs):
        if casaVersion >= '5.9.9':
            with pb.style.context('classic'):
                pb.rc('axes.formatter', useoffset=False) # CAS-13419
                pb.rc('axes', grid=True)                 # CAS-13419
                pb.rc('axes.grid', axis='both', which='major') # CAS-13419
...
def plotbandpass3(caltable='', antenna='', field='', spw='', yaxis='amp',
                  ...
                  scans='', figfileSequential=False, groupByBaseband=False,  # 52 args
                  ...
                  firstPlot=0, Feff=0.99, SBGain=0.99, Trx='auto', 
                  showtsys=False, atmType=1, preferredStation='TB2,Central', h0=1.0): # 69 args
  """
  ...
  if (help):
      ...
      print("   ycolor='g', chanrangeSetXrange=False, atmType=1)")
      ...
      print(" atmType:   tropical=1, midLatitudeSummer=2, midLatitudeWinter=3")
      ...
      print(" density: dpi to use in creating PNGs and PDFs (default=108); deprecated in CASA 6.5.3")
      ...
      print(" h0: water vapor scale height (in km) CASA default is 2 km")
      ...
      print(" preferredStation: a comma-delimited list of weather station names in priority order.")
      print("   Examples: 'Central' or 'MeteoCentral,TB2' (if none are found, the median values from all are used)")
...
#       This leaves a white blank space, but text no longer overlaps.
        if (os.path.basename(figfile).find('regression') < 0):
            figdesc.set_size_inches((11.2,8.4), forward=True)
        else:
            figdesc.set_size_inches((8.0,6.5),forward=True)
    else:
        figdesc.set_size_inches(8.0,6.5,forward=True)
...
#          showtsky = False
  ...
  pb.clf()
  if adesc is None:
      adesc = pb.subplot(111)  # needed in CASA 6.4
  figdesc = pb.gcf()
  if casaVersion >= '5.9':
#     This leaves a white blank space, but text no longer overlaps.
      if (os.path.basename(figfile).find('regression') < 0):
          ...
          figdesc.set_size_inches(8, 6.5, forward=True)
  else:
      figdesc.set_size_inches(8.0,6.5,forward=True)
...
#      print "madstats = ", madstats
  ...
  if (basebands == []):
      ...
      print("spwsToPlotInBaseband = ", spwsToPlotInBaseband)
...
#        print("**** Testing newpage(%d) == 1" % (newpage))
        if (newpage==1):
...
#                                 mytime, scansToPlot, scansForUniqueTimes, myprint=False) # task version
        ...
        if (not matchFound and newpage==0):
          ...
          if (len(pages)>1) \
            and (subplot==11 or (subplot!=11 and firstTimeMatch==-1 and firstSpwMatch==-1)):
...
#            print "Applying fix for CAS-7753"
            newpage = 1
            pages = pages[:len(pages)-1]
            if debug: print("Reducing pages to length=%d" % (len(pages)))
...
#          if (refFreq[myspw]*1e-9 > np.mean(frequencies)):
        ...
        if ((showatm or showtsky) and (len(xchannels)>1 or len(ychannels)>1) and
          ...
          if False: 
              ...
              print("             computedAtmField=%d, fieldIndex=%d;  computedAtmSpw=%d, ispw=%d;  computedAtmTime=%d,mytime=%d, xant=%d, antenna=%s" % (computedAtmField, fieldIndex, computedAtmSpw, ispw, computedAtmTime,mytime,xant,antstring))
...
#            print "Running CalcAtm:  CAF=%d, CAS=%d, CAT=%d, xant=%d" % (computedAtmField, computedAtmSpw, computedAtmTime, xant)
            ...
            (atmfreq,atmchan,transmission,pwvmean,atmairmass,TebbSky,missingCalWVRErrorPrinted,Tsys) = \
               CalcAtmTransmission(channels, frequencies, xaxis, pwv,
                                   ...
                                   verbose=DEBUG, maxAtmCalcChannels=maxAtmCalcChannels, h0=h0,
                                   maxAltitude=maxAltitude, atmType=atmType, Feff=Feff, SBGain=SBGain, 
                                   Trx=Trx, showtsys=showtsys, preferredStation=preferredStation)
            if showtsys: 
                TebbSky = Tsys
            if (showimage):
                if (lo1 is not None):
            ...
            if (LO1 is not None):
                ...
                (atmfreqImage,atmchanImage,transmissionImage,pwvmean,atmairmass,TebbSkyImage,missingCalWVRErrorPrinted,TsysImage) = \
                    CalcAtmTransmission(channels, frequenciesImage, xaxis,
                                        ...
                                        maxAtmCalcChannels=maxAtmCalcChannels, h0=h0,
                                        maxAltitude=maxAltitude, Feff=Feff, SBGain=SBGain,
                                        Trx=Trx, showtsys=showtsys, atmType=atmType,
...
#        flagfrequencies = [frequencies, frequencies2]
        ...
        # you never get here if overlayAntennas and final antenna was flagged on this spw: CAS-13568
...
#                    sTPPS = [scansToPlot[ispw][-1]]# added [[-1]]  on 2014-04-04   task version
                ...
                if (sloppyMatch(timerangeListTimes[-1], uniqueTimes[mytime],
                    ...
                    if (overlayAntennas == False or xant==antennasToPlot[-1]):  # 11-Mar-2014
                        if debug:
                            print("**** Setting doneOverlayTime = True (a)")
...
#                  (spwctr==lowestSpwInFirstScan and str(xframe)[-1] == '0') or 
            ...
            if casaVersion < '5.9.9':
                adesc = safe_subplot(xframe)  # there is no deprecation warning in older CASA
            if (previousSubplot != xframe):
                if casaVersion >= '5.9.9':
                    adesc = safe_subplot(xframe)  # avoid deprecation warning in CASA6 when xframe already was opened
                drewAtmosphere = False
            previousSubplot = xframe
...
#            print "amp: ylim, yrange = ",  ylim, yrange
            if (overlayAntennas == False and overlayTimes == False and bOverlay == False and 
                ...
                if debug:
                    print("**** Setting doneOverlayTime = True (b)")
                for f in fieldIndicesToPlot:
                  if (len(uniqueTimesPerFieldPerSpw[ispwInCalTable][f]) > 0):
                    if ((uniqueTimes[mytime] < uniqueTimesPerFieldPerSpw[ispwInCalTable][f][-1]-solutionTimeThresholdSeconds) and
                        ...
                        and scansForUniqueTimes[mytime] != scansForUniqueTimes[-1]  # fix for CAS-14096
                    ):
                        if (debug):
...
#                if (xframe == firstFrame and mytime == 0 and xctr==firstUnflaggedAntennaToPlot and bOverlay==False):  
                ...
                if (xant==antennasToPlot[-1]):
                    if debug:
                        print("**** Setting doneOverlayTime = True (c)")
                    doneOverlayTime = True  # assumed until proven otherwise in the 'for' loop
                    for f in fieldIndicesToPlot:
                        if (len(uniqueTimesPerFieldPerSpw[ispwInCalTable][f]) > 0):
                            if ((uniqueTimes[mytime] < uniqueTimesPerFieldPerSpw[ispwInCalTable][f][-1]-solutionTimeThresholdSeconds) and
...
#                        print "Checking if %f >= %f" % (xrange2, myxrange)
            ...
            if debug:
                print("***** Checking whether I need to draw the atmosphere and FDM windows")
...
#                 xant==antennasToPlot[-1] and doneOverlayTime and mytime==nUniqueTimes-1 
                ...
                if (overlayTimes and not overlayAntennas and debug):
                    print("doneOverlayTime=%s, mytime=%d" % (str(doneOverlayTime), mytime))
            ...
            matched,mymatch = sloppyMatch(myIndexTime,uniqueTimes,
                                          ...
                                          whichone=True)
            if (debug):
                print("1)done sloppyMatch, mytime=%d, scansForUniqueTimes=%s" % (mytime,str(np.unique(scansForUniqueTimes))))
                print("ispw=%d" % (ispw))
...
#            print "phase: ylim, yrange = ",  ylim, yrange
            ...
            if (overlayAntennas == False and overlayTimes == False and bOverlay == False and
                ...
                doneOverlayTime = True # assumed until proven otherwise in the 'for' loop
                if debug:
                    print("**** Setting doneOverlayTime = True (d)")
                for f in fieldIndicesToPlot:
                    if (uniqueTimes[mytime] < uniqueTimesPerFieldPerSpw[ispwInCalTable][f][-1]-solutionTimeThresholdSeconds and
                        uniqueTimes[mytime] < timerangeListTimes[-1]
                        and scansForUniqueTimes[mytime] != scansForUniqueTimes[-1]):  # fix for CAS-14096
            ...
            if ((overlayAntennas==False and overlayTimes==False) or 
                ...
                if overlayTimes and doneOverlayTime and debug:
                    print("**** Done overlayTimes, so not showing Atm or FDM.")
...
#                  print "len(scansToPlotPerSpw[ispw]) = %d" % (len(scansToPlotPerSpw[ispw]))
          if (scansForUniqueTimes == []):
              ...
              if (debug):
                  ...
                  scansToPlotPerSpwUnique = {}
                  for sc in list(scansToPlotPerSpw.keys()):
                      scansToPlotPerSpwUnique[sc] = np.unique(scansToPlotPerSpw[sc])
                  print("scansToPlotPerSpw = ", scansToPlotPerSpwUnique)
...
def CalcAtmTransmission(chans,freqs,xaxis,pwv,vm,vis,asdm,antenna,timestamp,
                        ...
                        Feff=0.99, SBGain=0.99, Tamb=273.0, Trx=None, 
                        showtsys=False, preferredStation='TB2'):
    """
    ...
    atmType:   tropical=1, midLatitudeSummer=2, midLatitudeWinter=3=(CASA default)
    Trx: receiver temperature to use when computing Tsys, None -> use au.receiverTrxSpec
    preferredStation: e.g. 'TB2' or 'MeteoTB2'; 
         '' means use median of all stations; 
         if the P or H values are <=0, then a sensible default will be used
    """
    if (casaVersion >= '4.1.0' and mymsmd != ''):
...
#            print "done"
    ...
    if (vm != '' or mymsmd != ''):
        if (casaVersion >= '4.4' and casaVersion < '4.6'):
            weatherResult = au.getWeather(vis,bestscan,antenna,verbose,vm,mymsmd,obsid=bestscan_obsid,preferredStation=preferredStation,getSolarDirection=False)
        else:
            weatherResult = au.getWeather(vis,bestscan,antenna,verbose,vm,mymsmd,
                                          preferredStation=preferredStation,
                                          getSolarDirection=False)
    else:
        weatherResult = None
    ...
    if (P <= 0.0):
        print("Invalid pressure (%f), will use 563mb" % (P))
        P = 563
    if (H <= 0.0):
        print("Invalid humidity (%f), will use 20%%" % (H))
        H = 20
...
#    print("using pwvmean = %f, fCenter=%f GHz" % (pwvmean,reffreq))
...
#    print("getBasicAtmParms: %s" % str(myat.getBasicAtmParms()))
    if needToCloseAT:
        myat.close()
    myqa.done()
...
#                           rf-((numchan-1)/2.)*chansepGHz, numchan)
    print("Median transmission = %.2f%%" % (100*np.median(transmission)))
...
def sloppyMatch(newvalue, mylist, threshold, mytime=None, scansToPlot=[],
    ...
    debug = myprint
    if (debug):
        print("sloppyMatch: scansToPlot = %s" % (str(np.unique(scansToPlot))))
    mymatch = None
    if (len(scansToPlot) > 0):
        ...
        if (matched == False and myprint==True):
            print("sloppyMatch: %d is not in %s" % (myscan, np.unique(list(scansToPlot))))
        elif (myprint==True):
            print("sloppyMatch: %d is in %s" % (myscan, np.unique(list(scansToPlot))))
    else:
        matched = False
